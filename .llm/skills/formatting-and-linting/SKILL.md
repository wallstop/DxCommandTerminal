...
description: Run CSharpier and the repo's pre-commit enforcement for DxCommandTerminal (dotnet tools, formatting C# files, fixing format-check failures). Use when formatting code, adding pre-commit hooks, or when a commit is rejected by the csharpier or dotnet-tool-restore hooks.
metadata:
  category: Core
---
...
3. LLM-context linters: `lint-llm-instructions.ps1`, `lint-skill-sizes.ps1` (see
