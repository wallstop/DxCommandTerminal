/*
    ...
    Imports the actual release artifact into a throwaway Unity project with
    two batch-mode Unity invocations and fail-closed validates the result on
    disk. This is a local, maintainer-run gate: it uses the local Unity
    license and network access to resolve the package's UPM dependencies, and
    it never runs in CI.

    Phase 1 imports through the documented CLI argument
    (`Unity -batchmode -nographics -quit -projectPath <dir> -importPackage
    <artifact>`). Phase 2 runs a generated settle driver
    (-executeMethod) that waits out the import-triggered compilation and
    exits 0 once the asset pipeline is idle.

    Why not `AssetDatabase.ImportPackage` in the driver? Measured on Unity
    6000.4.6f1 (session-034): in batch mode the API silently no-ops the
    ENTIRE package - no error, no log, exit 0 - whenever any entry's
    ...
    import, a hard timeout, and separate processes.
...
public static class DxTerminalImportDrill
{
    ...
    private static void WaitForImportAndCompile()
    {
        ...
        Debug.Log("[import-drill] import and compile settled");
        EditorApplication.update -= WaitForImportAndCompile;
...
/*
    Shapes the two Unity invocations. Phase 1 imports the artifact through
    Unity's documented CLI argument (batch-mode AssetDatabase.ImportPackage
    silently drops the whole package when any pathname targets Packages/ -
    measured on 6000.4.6f1, see the header note). Phase 2 waits out the
    import-triggered compilation.
*/
export function importUnityArgs(projectDir, artifactPath, logPath) {
  return [
    "-batchmode",
    "-nographics",
    "-quit",
...
export function settleUnityArgs(projectDir, logPath) {
  return [
    "-batchmode",
    "-nographics",
    "-executeMethod",
...
export async function runImportDrill(options, probes = {}) {
  const runUnity = probes.runUnity ?? runUnityProcess;
  ...
  const importLog = path.join(reportDir, "unity-import.log");
  const settleLog = path.join(reportDir, "unity-settle.log");
  ...
  const totalBudgetMs = options.timeoutMinutes * 60_000;
  console.log("[import-drill] phase 1: importing (CLI -importPackage)");
  const importRun = await runUnity(
    options.unity,
    importUnityArgs(projectDir, options.artifact, importLog),
    importLog,
    totalBudgetMs
  );
  let settleRun = null;
  let outcome = phaseOutcome(importRun, "unity import phase");
  if (outcome === null) {
  ...
  const logErrors = scanLogForErrors(importLog);
  if (settleRun !== null) {
    logErrors.push(...scanLogForErrors(settleLog));
  }
  ...
  const manifest = {
    ...
    editorVersion,
    project: projectDir,
    unityImport: phaseRecord(importRun, importLog),
    unitySettle: settleRun === null ? null : phaseRecord(settleRun, settleLog),
    timedOut: importRun.timedOut === true || settleRun?.timedOut === true,
    elapsedSeconds,
    outcome,
  ...
  if (outcome !== null) {
    ...
    console.error(`[import-drill] logs: ${importLog} ${settleRun === null ? "" : settleLog}`);
...
function phaseOutcome(run, phase) {
  if (run.timedOut === true) {
    return `${phase} timed out (process killed)`;
  }
  if (run.spawnError !== undefined) {
    return `could not launch unity: ${run.spawnError}`;
  }
  if (run.signal !== null && run.signal !== undefined) {
    return `${phase} killed by signal ${run.signal}`;
  }
  if (run.code !== 0) {
