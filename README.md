...
Stack-trace capture (`stackTraceMode`): `All` (default) records a caller stack trace for every log entry, exactly as before. `ErrorsAndWarnings` records traces only for error, assert, exception, and warning entries. `Disabled` records none. Trace extraction is the most expensive part of a logged line, so titles that log frequently during gameplay (per-frame state dumps, command chatter) can drop it for routine messages without losing diagnostics when something actually goes wrong.
