...
try {
    & pwsh -NoProfile -File $generateScript -OutputPath $tempA -RepoRoot $RepoRoot | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-ErrorMsg "Generator failed (exit $LASTEXITCODE) writing expected index."
        exit 1
    }
    & pwsh -NoProfile -File $generateScript -OutputPath $tempB -RepoRoot $RepoRoot | Out-Null
    if ($LASTEXITCODE -ne 0) {
    ...
    if (-not (Test-Path -LiteralPath $indexFile)) {
        if ($Fix) {
            & pwsh -NoProfile -File $generateScript -RepoRoot $RepoRoot | Out-Null
            if ($LASTEXITCODE -ne 0) {
