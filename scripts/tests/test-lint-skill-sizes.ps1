...
$sizeLinter = (Get-Item (Join-Path $PSScriptRoot '../lint-skill-sizes.ps1')).FullName
...
Invoke-TestCase 'files at or under 269 lines pass' {
    ...
    try {
        ...
        $result = Invoke-PwshScript -Path $sizeLinter -Arguments @('-RepoRoot', $root, '-VerboseOutput')
...
Invoke-TestCase 'file at exactly 270 lines is CRITICAL but exits 0' {
    ...
    try {
        ...
        $result = Invoke-PwshScript -Path $sizeLinter -Arguments @('-RepoRoot', $root)
...
Invoke-TestCase '-FailOnCritical promotes the critical band to a failure' {
    ...
    try {
        ...
        $result = Invoke-PwshScript -Path $sizeLinter -Arguments @('-RepoRoot', $root, '-FailOnCritical')
