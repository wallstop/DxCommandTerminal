...
- Context-aware execution (T08): new `CommandExecutionContexts`, `CommandExecutionContext`, `CommandDefinition`, `CommandHandler`, `BorrowedCommandArguments` types, registered through `CommandShell.AddCommand(CommandDefinition)`. Definitions default to gameplay contexts; Edit Mode execution is opt-in. `[RegisterCommand]` gains a `Contexts` property (default `All`, so existing attributed commands keep their availability).
- Argument completion providers: `CommandShell.TryComplete` builds a `CommandCompletionContext` for the command under the caret and fills a caller-owned buffer. Provider exceptions are contained; results dedupe ordinally. Commands without a provider keep the previous history-based completion.
