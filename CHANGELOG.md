...
- The package ships a `Runtime/link.xml` that preserves its runtime assembly through managed code stripping (IL2CPP/WebGL). Built-in commands and the generated command catalogs now survive every Managed Stripping Level; commands declared in your own assemblies still need your own stripping configuration (`[Preserve]`, your own `link.xml`, or manual registration) at `Medium` or higher (see the README).
